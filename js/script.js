...
function addTask() {
  if (noTxt()) { // 입력 값이 있는지 확인합니다.
    ...
    // 할일을 추가한 후에 현재 선택된 탭에 따라 필터링된 목록을 다시 렌더링합니다.
...
// 여기서 문제
// on_going 탭을 누른 상태에서 할일 입력 시 화면에서 보이지 않고 all 탭으로 이동 후 확인해야 함
// 할 일을 추가한 후에도 현재 선택된 탭에 따라 화면이 제대로 업데이트되지 않음
// 수정 방안은?
// 할 일을 추가한 후에 현재 선택된 탭에 맞게 화면을 다시 렌더링 해야 함
// 할 일을 추가한 후에는 현재 선택된 탭에 따라 목록을 필터링하고 다시 렌더링
// 1. addTask() 함수 내에서 할 일을 추가한 후에 filter() 함수를 호출
// 2. 현재 선택된 탭에 따라 목록을 필터링
// 3. 필터링된 목록을 다시 화면에 렌더링